...
from .base import BaseStartArgs, Message, AbstractConnection, AbstractChannel, AbstractClient
...
class WebsocketConnection(AbstractConnection):
    ...
    async def listen(self):
        while True:
            ...
            if data.lower() == str(MessageEnum.close):
                # Users disconnected
                raise CloseSessionException("User closed the session")
            elif data.lower() == str(MessageEnum.ping):
                # App checks if alive
                continue
